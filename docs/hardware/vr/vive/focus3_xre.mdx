...
The Vive Focus Vision, Focus 3 and XR Elite are standalone VR headsets powered by the Qualcomm XR2, similar to the Quest 2 and Quest Pro headsets from Meta.
The Focus Vision is a slightly upgraded Focus 3 with built-in eye tracking (of the same form as the Focus 3 eye tracker) and newer software. 
The Focus 3 has 2 add-on modules that can be installed to add eye and face tracking capabilities, and the XRE has a single combo module that can be installed to add both eye and face tracking capabilities.
While these headsets can send a (extremely) limited set of face tracking parameters (a total of 5 parmeters, and a max of 3 which can work simultaneously) to the Vive standalone version of VRChat, this function is unrelated to VRCFaceTracking and questions/issues regarding this headset feature should be directed to Vive Support. 
...
While users can choose to forgo using VRCFT, we would recommend using VRCFT over the Vive Streamer's built-in OSC function (Vive themselves now also recommend using VRCFT over their native output).
...
<img src={require("./img/rr/streamer_beta.png").default} alt="Vive Streamer Beta option" />
</div>
::: -->
...
2. Update the streaming app to the latest version on the Focus Vision, Focus 3, or XR Elite by plugging the headset into the computer then clicking the Update button in the VIVE Streaming application for "Headset software version" (Settings ➜ About ➜ Vive Streaming).
  ...
  <div style={{
    ...
    <img src={require("./img/rr/streamer_update.png").default} alt="Headset software version Update button" />
  </div>
...
<div style={{
    ...
    <img src={require("./img/rr/streamer_settings.png").default} alt="Vive Streamer software settings" />
</div> -->
...
<div style={{
    ...
    <img src={require("./img/rr/streamer_osc_toggle.png").default} alt="Vive Streaming Eye and Face tracking toggle" />
</div>

4. Install the **ViveStreamingFaceTrackingModule** from the VRCFaceTracking module registry.
...
<details>
  ...
  SRanipal was the original ET/FT method that was available for the Focus 3 headset, and still works for any of their current standalone headsets.
